...
9. Push branch and create PR automatically (requires `gh` CLI)
...
2. Creates a pull request using `gh pr create` (or retrieves existing PR URL)
3. Outputs the PR URL for review
4. Outputs `<promise>COMPLETE</promise>` and the loop exits

If `gh` CLI is not available, Ralph reports manual PR instructions and still exits successfully.
